...
{
    public partial class TipoFormula_Form : Form
    {
        TIPOFORMULA tipoFormula;
        TypeFormulateBL tipoFormulaBL;


        /// <summary>
        /// Constructor para inicializar atributos y elementos del formulario
        /// </summary>
        public TipoFormula_Form()
        {
            try
            {
                InitializeComponent();
        ...
        private void Listado_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (Listado.Columns[e.ColumnIndex].Name == "Editar")
                {
                    ...
                    tipoFormula = tipoFormulaBL.GetTypeFormulate((Guid)row.Cells[0].Value);

                    updateNombreTxt.Text = tipoFormula.NOMBRETIPO;
                    updateDescripTxt.Text = tipoFormula.DESCRIPCION;
                    pn_listado.Hide();
                    pn_crear.Hide();
                    pn_editar.Show();
                ...
                if (Listado.Columns[e.ColumnIndex].Name == "Eliminar")
                {
                    ...
                    switch (result)
                    {
                        case DialogResult.OK:
                            tipoFormulaBL.DeleteTypeFormulate(tipoFormula.NROTIPOFORMULA);
                            string resultD = tipoFormulaBL.SaveChanges();
                            if (resultD == "")
                            {
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (updateDescripTxt.Text != "" && updateNombreTxt.Text=="")
                {
                    eP_errors.Clear();
                    tipoFormula.NOMBRETIPO = updateNombreTxt.Text;
                    tipoFormula.DESCRIPCION = updateDescripTxt.Text;
                    tipoFormulaBL.UpdateTypeFormulate(tipoFormula);
                    string resultUpd = tipoFormulaBL.SaveChanges();
                    if (resultUpd == "")
                    {
                ...
                else
                {
                    ...
                    if (updateNombreTxt.Text == "") eP_errors.SetError(updateNombreTxt, "Ingrese un nombre, este campo es requerido");
                }
            }
            catch (Exception ex)
            {
        ...
        private void busquedaTxt_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (busquedaTxt.Text != "")
                {
                    ...
                    {
                        Listado.DataSource = tipoFormulaBL.SearchTypeFormulate(busquedaTxt.Text, "DESCRIPCION");
                        Listado.Refresh();
                    }
                    if (criteria == "Nombre")
                    {
                        Listado.DataSource = tipoFormulaBL.SearchTypeFormulate(busquedaTxt.Text, "NOMBRE");
                        Listado.Refresh();
                    }
                ...
                {
                    Listado.DataSource = tipoFormulaBL.GetTypeFormulates();
                    Listado.Refresh();
                }
            }
            catch (Exception ex)
